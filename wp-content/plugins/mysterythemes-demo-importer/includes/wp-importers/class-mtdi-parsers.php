...
class MTDI_Parser {
	function parse( $file ) {
		...
		if ( extension_loaded( 'simplexml' ) ) {
			...
			$parser = new MTDI_Parser_SimpleXML;
		...
		if ( isset( $result ) && defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
			...
			echo '<p><strong>' . __( 'There was an error when reading this WXR file', 'mysterythemes-demo-importer' ) . '</strong><br />';

			echo __( 'Details are shown above. The importer will now try again with a different parser...', 'mysterythemes-demo-importer' ) . '</p>';

		}

		// use regular expressions if nothing else available or this is bad XML
		$parser = new MTDI_Parser_Regex;
...
class MTDI_Parser_SimpleXML {
	...
	function parse( $file ) {
		...
		if ( ! $success || isset( $dom->doctype ) ) {
			return new WP_Error( 'SimpleXML_parse_error', __( 'There was an error when reading this WXR file', 'mysterythemes-demo-importer' ), libxml_get_errors() );
		}
		...
		if ( ! $wxr_version ) {
			return new WP_Error( 'WXR_parse_error', __( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'mysterythemes-demo-importer' ) );
		}
